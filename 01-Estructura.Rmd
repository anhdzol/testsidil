...
cap_num <- as.numeric(gsub("\\D", "", rstudioapi::getActiveDocumentContext()$path))+1

fn = local({
  i = 0
  function(x) {
    i <- i + 1
    paste('Recuadro ', cap_num,'.', i, ': ', x, sep = '')
  }
})
...
```{r pipelsimplaaificado2, echo=FALSE, out.width="100%", fig.cap ="Pipeline simplificado del SIDIL  (versión simplificada)"}
#Nota, los nombre de referencia no pueden tener guion bajo

knitr::include_graphics("images-1/02/pipeline-simplificado.jpg")
...
```{r scriptinputoutput,echo=FALSE, out.width="50%", fig.cap = 'Relación input-script-output'}

knitr::include_graphics("images-1/02/relacion-input-script-output.jpg")
...
A su vez, la modularización se observa, cuando es posible, al interior mismo de los módulos. Por ejemplo, si hubiera un cambio en uno de los campos de una fuente de información que impidiera el cálculo de cierto indicador, esto no inhibe la actualización de otros indicadores de dicha fuente de información.
...
El SIDIL tiene cuatro módulos, tal como se muestra en el recuadro \@ref(fig:modulosSIDIL): El [primer módulo](#moduloIexplicaciongeneral) es el de actualización de indicadores, mismo que explota las fuentes de información exógenas; el [segundo módulo](#moduloIIexplicaciongeneral) es el de consulta al universo de CT registrados en el DNE, la imputación de indicadores y de predicción de riesgo, que incorpora los resultados del módulo anterior. Estos dos módulos son operables a través de una [interfaz de automatización](#moduloIyIIexplicaciongeneral), misma que a su vez permite de una manera muy simple, asegurando coherencia y consistencia, la incorporación y actualización de catálogos y fuentes de información.

Así, la interfaz de automatización abarca el desarrollo de los procesos que están en color amarillo en esta ilustración. Luego, el tercer módulo es el de la [interfaz de consulta](#interfazConsultaexplicaciongeneral), que explota el principal resultado del módulo anterior (la Matriz de Predicción de Riesgos, MPR). Esta consulta está esquematizada en el área de color rojo tinto de la figura, y da como resultado un *listado de centros de trabajo priorizados*, listos para ser incorporados a la programación de inspecciones. Por último, el cuarto módulo, de entrenamiento, es el que permite reentrenar el modelo para actualizar la forma en la que se realizará la predicción. El módulo cuarto arroja un modelo predictivo entrenado (llamémoslo "parámetros del modelo") que el módulo II consulta para la predicción del modelo sobre el universo de CT enriquecido con los indicadores.

En resumen, cada uno de los módulos tiene un objetivo y alcance definido:

* Módulo I: explotación de fuentes de información para la actualización de indicadores laborales. 
* Módulo II: imputación de los indicadores calculados más recientemente en el módulo anterior al universo de los CT obtenidos del DNE, para con ello poder aplicar el modelo entrenado que permite la actualización de la MPR.
* Interfaz de automatización de procesos: Es la interfaz que permite la actualización de insumos y catálogos y la ejecución de los procesos estipulados en los módulos I y II.
* Módulo III: Es la interfaz que permite a la DGIFT la consulta a la MPR para la priorización de los CT con base en las predicciones de riesgo, indicadores imputados a los CT y sus atributos. Notar que en este módulo no hay scripts para la explotación de datos.
* Módulo IV: Relaciona los antecedentes de inspección con la serie histórica de los indicadores laborales y atributos de los CT para entrenar un modelo de inteligencia artificial. Su producto es solamente el modelo entrenado, mismo que se aplica o implementa en el módulo II. 


```{r modulosSIDIL, echo=FALSE ,out.width="100%", fig.cap ="Módulos que componen el SIDIL"}
#Nota, los nombre de referencia no pueden tener guion bajo
knitr::include_graphics("images-1/02/modulos-SIDIL.jpg")
# print("aqui PIPELINE")

```


```{r modularizacionSIDIL, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="Modularización del SIDIL", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
# htmltools::HTML(fn('Modularización del SIDIL'))
```
...
A continuación, se ofrecen las principales características de estos módulos.
...
<!-- [@@ idea videocapsula: breve tour por el catalogo de indicadores, mostrando como de lo más básico (promedio nacional) llegas a lo más granular: 3_geo5_act3_tam ]{.mark} -->


```{r aprovechamientoNivelesAgregacion,echo=FALSE, out.width="100%", fig.cap="El aprovechamiento de los niveles de agregación en el cálculo de indicadores"}
knitr::include_graphics("images-1/01/aprovechamientoNivelesagregacion.jpg")
```

```{r utilidadestimacionespuntuales, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="El aprovechamiento de los niveles de agregación en el cálculo de indicadores.", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
...
```{block, type='rmdcomment', fig.cap="test numeracion block"}
 Sirve dar un ejemplo de la utilidad de calcular las estimaciones puntuales de los indicadores a diferentes niveles de agregación: 
 
 Para el cálculo de la media de la tasa de prevalencia de asalariados percibiendo hasta 105% del salario mínimo (uno de los indicadores desarrollados) a nivel nacional, el promedio asciende a 12.8% de los puestos asegurados. 
 Esto significa que casi 13% de los asegurados en el IMSS perciben hasta 105% del salario mínimo. Este es el promedio nacional y es el menor nivel de agregación de o especificidad. Pero detrás de este promedio hay una elevada heterogeneidad: tan solo observando el siguiente nivel de agregación que distingue por categoría de tamaño se percibe que los centros de trabajo (CT) de hasta 10 personas aseguradas tienen una prevalencia del 21.7%, mientras que las de 251 o más asegurados se reduce a 0.77%. 
 Si además a los CT de menor tamaño se les distingue por la clasificación industrial a dos dígitos, se estima que aquellas en la categoría industrial SCIAN: 11 (Agricultura, cría y explotación de animales) tienen un promedio de 21.3% pero las SCIAN 21 (minería) un promedio de 16.9%. Este último nivel de agregación ya recupera la información de dos campos (tamaño y clasificación industrial a dos dígitos), pero puede profundizarse aún más incorporando la dimensión geográfica (entidad federativa y municipio) y además llevar a 3 dígitos la clasificación industrial. 
 De esta manera, a mayor incorporación de dimensiones, o mientras más específicas sean éstas más preciso es el dato. **A mayor precisión, mayor la posible heterogeneidad en la caracterización de los CT y mayor poder predictivo de los indicadores**.
```
...
```{r sentidoindicadores, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="El sentido de los indicadores", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
# htmltools::HTML(fn('Nombre del recuadro desde DIV...'))
```
...
El resultado del módulo I son diversas tablas en formato .Rds que contienen las estimaciones puntuales de todos los indicadores de todas las fuentes de información, así como la información sobre los niveles de agregación e información complementaria del cálculo. 

```{r MPRcomplementaria, echo=FALSE, fig.cap ="TInformación en las tablas de indicadores calculados en el Módulo I"}
#Nota, los nombre de referencia no pueden tener guion bajo

knitr::include_graphics("images-1/02/MPR_complementaria.jpg")
...
```{r resumenModII, echo=FALSE, out.width="100%", fig.cap="Explicación general Módulo II"}
knitr::include_graphics("images-1/02/resumenModII.jpg")
```

<!-- [@@ PENDIENTE AQU POSIBLEMENTE CONVIENE INCORPORAR UNA ILUSTRACION SOBRE RELACION SIAPI SIPAS, DNE Y SIDIL]{.mark} -->

```{r relacionDNESNILESIAPISIPAS, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="Relación DNE-SNILE-SIAPI-SIPAS con el matching nominativo del IMSS", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
# htmltools::HTML(fn('Nombre del recuadro desde DIV...'))
```
...
```{r matchingnominativoagregacion, echo=FALSE, out.width="100%", fig.cap="matchingnominativoagregacion"}
knitr::include_graphics("images-1/02/matchingnominativoagregacion.jpg")
```
...
```{r MPRsimplificada, echo=FALSE, fig.cap ="Matriz de predicción de riesgos (ilustración simplificada)"}
#Nota, los nombre de referencia no pueden tener guion bajo
...
```{r motivosparaactualizarMPR, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="La utilidad de los secuenciadores", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
# htmltools::HTML(fn('Nombre del recuadro desde DIV...'))
```
...
```{block, type='rmdcomment', fig.cap="test numeracion block"}
¿Bajo qué circunstancias es importante actualizar la MPR? Sin ignorar el hecho de que el universo de CT registrados en el DNE puede variar día a día (todos los días se podría estar incorporando un nuevo CT), pueden distinguirse tres circunstancias específicas en las cuales se vuelve realmente importante actualizar la MPR:

1. Cuando existen variaciones sustantivas en los valores coyunturales de las estimaciones puntuales de los indicadores laborales calculados en el módulo I. Esto por ejemplo, se vuelve crítico cuando se presentan cambios abruptos en el mercado laboral (por ejemplo, en la reciente pandemia), pero también es importante reconocer cierta ciclicidad de algunos indicadores o algunos efectos de cambios en la política laboral. Un claro ejemplo de esto es el cálculo de la prevalencia de asalariados percibiendo cerca de un salario mínimo, lo cual, presenta un salto discreto al inicio de cada año. Por lo anterior, dada la frecuencia de publicación de las actualizaciones de la ENOE, se sugiere que, sólo desde esta perspectiva, la MPR se actualice cada tres meses.

2. Cuando se ha realizado un nuevo entrenamiento del modelo en el módulo IV. Esto podría haber sido producto de una recalibración del mismo, la necesidad de predecir sobre nuevas submaterias, etc. mismo que, de no volver a predecir la MPR, no será reflejado en la misma.

3. Cuando se presenten cambios sustantivos en el número y la composición de los CT registrados en el DNE. Esto pudiera ser por incorporaciones o depuraciones masivas en el mismo, cambios en la generación de algunos campos, o simplemente, el natural paso del tiempo y crecimiento en el número de registros allí encontrados.


En cualquiera de estas tres circunstancias se vuelve importante sino imprescindible volver a correr todos los scripts que se relacionen con el origen de dichos cambios: a) si hay cambios o actualizaciones en la ENOE, correr el módulo I (script secuenciador de la ENOE) y todos los posteriores que se encuentran en el módulo II, b) si hay cambios en el módulo IV, volver a correr el módulo II (script de predicción de riesgos), c) si hay cambios en la composición del universo de CT, todos los scripts del módulo II.  
...
```{r utilizacioninterfazauto, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="La utilización de la interfaz de automatización", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
# htmltools::HTML(fn('Nombre del recuadro desde DIV...'))
```
...
```{r entrenamientoModelo, echo=FALSE, out.width="100%", fig.cap="Entrenamiento del modelo"}
knitr::include_graphics("images-1/02/entrenamientoModelo.jpg")
```
...
<!-- [Placeholder imagen pixelada pendiente mejora]{.mark} -->

```{r arboldecision, echo=FALSE, eval=FALSE, fig.cap ="Ejemplo de un árbol de decisión"}
#Nota, los nombre de referencia no pueden tener guion bajo

knitr::include_graphics("images-1/02/arbol_decision.png")
...
```{r frecuenciaModIV, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="La utilidad de los secuenciadores", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
# htmltools::HTML(fn('Nombre del recuadro desde DIV...'))
```
...
```{block, type='rmdcomment', fig.cap="test numeracion block"}
¿Con qué frecuencia debe actualizarse el módulo IV? Esta pregunta podría tener diferentes respuestas, según diversos factores como la disponibilidad de tiempo del personal de la DGIET, la precisión con la que se quiera operar el SIDIL, el volumen y la distribución de las inspecciones que se realicen, etc. Poniendo en la balanza estos y otros factores, cabe sugerir que el módulo IV, el cual es el computacionalmente más intensivo dado el periodo temporal que cubre, no requiere ser actualizado con la misma frecuencia que los demás módulos (mismo que se propone sea cada tres meses). Esto porque, todo lo demás constante, no es sustantiva la aportación que realice al entrenamiento del modelo la incorporación de tres, seis o incluso nueve meses de inspección, claro está, salvo que éstas sean de un volumen considerable o representen un cambio sustantivo respecto a los antecedentes. Podría pensarse que el módulo IV puede actualizarse cada uno o dos años, a menos que se observe la ocurrencia de algunos eventos como los siguientes:

1. Que se identifique que la predicción del modelo ha empeorado y que se requiere reentrenar el mismo probando otra configuración de hiperparámetros.
2. Que se requiera predecir el riesgo de incumplimiento en ciertas submaterias sobre las cuales no hay predicciones actualmente.
3. Que cambien los criterios y/o procedimientos para determinar las violaciones que proceden en los CT, o que se observe un cambio abrupto en los porcentajes de violaciones que procedene. 
4. Que se incorporen nuevos indicadores laborales o atributos sobre los CT en el DNE, y se pretenda incluir estos como variables de entrada en el modelo predictivo.
