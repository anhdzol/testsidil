...
```{r setup0311, include=FALSE, echo=FALSE}
fn = local({
  i = 0
  function(x) {
    i <<- i + 1
...
```{r precauciones, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="Aspectos a considerar al ejecutar los scripts en los módulos I", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
...
El aprovechamiento de bases de datos externas, o para el caso, de cualquier base de datos no es libre de riesgos. El principal riesgo es no detectar si ocurren cambios en las mismas y que éstos se  afecten los resultados o incluso las conclusiones que se deriven de dicho análisis. Se requiere mucha cautela y análisis crítico para detectar estos cambios. Por fortuna, en muchos casos las fuentes de información, como por ejemplo el INEGI, documentan los cambios que le realizan a sus proyectos estadísticos. 

A continuación se mencionan sólo algunos de los elementos que podrían formar parte de la rutina de cualquier área estadística en el procesamiento de datos:

1. Revisar la documentación de cada publicación o actualización de la base de datos. Prestar atención a cambios en los nombres de las variables, metodologías de levantamiento o de postprocesamiento de los datos, recálculo o ajustes a los campos que caracterizan el muestreo complejo, etc. 
2. Revisar si el número de observaciones y de población estimada presenta desviaciones sustantivas a las habitualmente obtenidas con esta misma base de datos.
3. Revisar si proyectos auxiliares relacionados tuvieron actualizaciones importantes que pudieran afectar la base de datos que se pretende explotar. Por ejemplo, el censo demográfico que permite actualizar los factores de expansión de la ENOE.
4. Revisar si las estimaciones de ciertas proporciones y poblaciones son similares a las obtenidas, a menos que haya motivo fundado para esperar cambios sustantivos, generalmente ciertos indicadores permanecen relativamente estables. En el caso del IMSS, por ejemplo, es la proporción de trabajadores en la frontera, o la participación de las mujeres en los puestos asegurados etc. 
5. Identificar si los indicadores que se obtienen presentan un comportamiento anómalo respecto a la serie histórica o ciertos valores de referencia. 

Para muchas circunstancias como, por ejemplo, el cambio de nombres de columnas, cambios en el marco geoestadístico, en la clasificación industrial, etc, el SIDIL contempla la posibilidad de atender estos cambios a través de los catálogos que para tal fin se incluyeron en el SIDIL. Ver  la sección [Catálogos de información](#catalogos) para más información. En otras circunstancias más específicas será necesario revisar con cautela la ejecución de los scripts, prestando atención a aquellos segmentos en los que aparecen los campos que sufrieron cambios que no se puedan atender con los catálogos. 
```
...
```{r IndicadoresNuevos1, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="Procedimiento para la inclusión de nuevos indicadores", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
```

```{block, type='rmdcomment', fig.cap="Procedimiento para la inclusión de nuevos indicadores"}
...
```{r nuevosypreexistentes1, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="Distinción entre incorporación de nuevos indicadores y actualización de método de cálculo de indicadores preexistentes", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
```

```{block, type='rmdcomment', fig.cap="Distinción entre incorporación de nuevos indicadores y actualización de método de cálculo de indicadores preexistentes"}
...
library(plotly)
library(tidyverse)
#seg_soc_hist <- readRDS("data/enoe_historico_2022_2023_07_06_1334.Rds") %>% filter(indicador=="seg_soc")

seg_soc_coy <- readRDS("data/enoe_historico_2022_2023_07_06_1334.Rds") %>% filter(indicador=="seg_soc") %>% filter(anio==max(anio)) %>% 
  select(valor,cv,agregacion,cat_tamanio,scian2018_2,scian2018_3,geo2,agregacion_numerica) %>% 
  filter(cv<100) %>% 
  group_by(agregacion) %>% 
  dplyr::slice_sample(prop=0.1,replace = TRUE) %>% 
  ungroup()  
  
p <- ggplot(data = seg_soc_coy, aes(x = (100*valor), y = cv,colour = agregacion,text =      paste("Valor:", 100*valor,"CV:",cv,"Agregacion:",agregacion,"Scian2:",scian2018_2,"Scian3:",scian2018_3,"Entidad:",geo2,"Cat. tamanio",cat_tamanio)))  +
  geom_point() +
  xlab("Porcentaje de acceso a la seguridad social")+
  ylab("coeficiente de variacion")+
  theme(legend.position="bottom")+
   guides(color=guide_legend(nrow=3, byrow=TRUE),
          size="none",
          alpha="none") +
  geom_smooth(data = seg_soc_coy %>% filter(agregacion_numerica==10),aes(x=(100*valor),y=cv), method = 'loess')
ggplotly(p, tooltip="text") # from ggplot to plotly

```

```{r modificacionagregacion1, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="Modificaciones a los niveles de agregación", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
```

```{block, type='rmdcomment', fig.cap="Modificaciones a los niveles de agregación"}
...
Como se observa en la sección de [Estructura y flujos de trabajo](#estructuracarpetas), la estructura de carpetas prevé una carpeta para cada indicador. A su interior, a su vez se incluye una subcarpeta llamada anteriores misma que permite almacenar los outputs de procesos anteriores.

```{r estimacionespuntuales1, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="Organización de las estimaciones puntuales por indicador", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
```

```{block, type='rmdcomment', fig.cap="Organización de las estimaciones puntuales por indicador"}
...
tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla1")

flextable::flextable(tabla1) %>%
   flextable::set_caption( caption = "Resumen de las fuentes de informacin utilizadas en el SIDIL", 
                           style = "Table Caption") %>% 
  flextable::footnote( i = 1, j = 4,  ref_symbols = "a",
  value = flextable::as_paragraph("Los detalles de las fuentes de informacion se presentan en el Modulo II")
) %>% 
...
```{r resguardobases1, class.source='myClass', echo=FALSE, out.width="50px", fig.cap=" Resguardo de las bases originales", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
```

```{block, type='rmdcomment', fig.cap=" Resguardo de las bases originales"}
...
```{r detallescriptsENOE, echo=FALSE, out.width="100%", fig.cap="Aplicación de scripts de la ENOE"}
knitr::include_graphics("images-1/04/detallescriptsENOE.jpg")
```
...
tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla2")%>% select(!(ends_with("lculo")&starts_with("C")))

flextable::flextable(tabla1) %>%
   flextable::set_caption( caption = "Indicadores calculados con base en la ENOE", 
                           style = "Table Caption") %>% 
...
[^procesamiento_fuentes-3]: Se homologan los nombres de las variables a las que utiliza el SIDIL dado que los nombres cambian según si se trata de un cuestionario ampliado (primeros trimestres del año) o no (el resto de trimestres del año
...
-   Su elevado nivel de cobertura: con tasas de informalidad que rondan el 55% a nivel nacional[^procesamiento_fuentes-4] esta información cubre cerca de la mitad de la población trabajadora. Por supuesto que, a diferencia del DNE, los CT registrados en la base de patrones únicamente son los empleadores formales[^procesamiento_fuentes-5].
...
[^procesamiento_fuentes-4]: Dato obtenido de la ENOE (INEGI) al cuarto trimestre de 2022, tasa de informalidad laboral I.

[^procesamiento_fuentes-5]: En referencia a la distinción de la informalidad (ver, entre otros, la matriz de Hussmann que periódicamente elabora el INEGI
...
```{r detallescriptsIMSS, echo=FALSE, out.width="100%", fig.cap="Aplicación de scripts del IMSS"}
knitr::include_graphics("images-1/04/detallescriptsIMSS.jpg")
```


##### Indicadores que se calculan {#indicadoresimss .unnumbered}

A continuación, se detallan los indicadores que se calculan a partir de los inputs que se reflejan en la siguiente subsección. Notar que todos los indicadores, incluidos los que se obtienen con datos del IMSS, antes del entrenamiento del modelo predictivo (Ver [Módulo IV](#procesamientoModIV) ) y de la predicción del mismo (Ver [Módulo II](#moduloii) ) se estandarizan entre 0 y 1 e invierten para expresar que "más es peor", es decir, un mayor percentil de un indicador se asocia a peores condiciones laborales.

```{r  echo = FALSE, message=FALSE,warnings=FALSE, results = 'asis'}
tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla4") %>% select(!(ends_with("lculo")&starts_with("C")))

flextable::flextable(tabla1) %>%
   flextable::set_caption( caption = "Indicadores que se calculan con base en los datos del IMSS", 
                  style = "Table Caption")%>% 
  theme_zebra(odd_body = "#EFEFEF", even_header = "transparent",even_body = "transparent") %>% 
  color(color = "#666666", part = "all") %>% 
  flextable::bg(i = 1, bg = "#FFFFFF", part = "header") %>% 
...
-   Base de puestos asegurados ante el IMSS, también de corte mensual y en formato de texto separado por tabulados (.tab)
    ...
    -   Ejemplo: PAFINALMES_F\_2022231.tab
...
```{r formatoperiodos1, class.source='myClass', echo=FALSE, out.width="50px", fig.cap=" Formato de periodo de referencia de los insumos del IMSS", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
```

```{block, type='rmdcomment', fig.cap="D Formato de periodo de referencia de los insumos del IMSS"}
...
```{r parametrosimss1, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="Configuración de parámetros del script secuenciador del IMSS", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
```

```{block, type='rmdcomment', fig.cap="Configuración de parámetros del script secuenciador del IMSS"}
...
```{r detallescenso, echo=FALSE, out.width="100%", fig.cap="Aplicación de scripts del Censo Económico 2019"}
knitr::include_graphics("images-1/04/detallescriptsCENSO.jpg")
```
...
tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla6")


flextable::flextable(tabla1) %>%
   flextable::set_caption( caption = "Indicadores que se calculan con base en el Censo Economico", style = "Table Caption")%>% 
  theme_zebra(odd_body = "#EFEFEF", even_header = "transparent",even_body = "transparent") %>% 
  color(color = "#666666", part = "all") %>% 
  flextable::bg(i = 1, bg = "#FFFFFF", part = "header") %>% 
  flextable::bold(bold = TRUE, part = "header") %>% 
...
tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla7") 

flextable::flextable(tabla1) %>%
   flextable::set_caption( caption = "Campos que se utilizan para el calculo de indicadores con base en el Censo Economico", style = "Table Caption")%>% 
  theme_zebra(odd_body = "#EFEFEF", even_header = "transparent",even_body = "transparent") %>% 
  color(color = "#666666", part = "all") %>% 
  flextable::bg(i = 1, bg = "#FFFFFF", part = "header") %>% 
  flextable::bold(bold = TRUE, part = "header") %>% 
...
tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla9") 

flextable::flextable(tabla1) %>%
   flextable::set_caption( caption = "Campos comunes en los resultados coyunturales del calculo de indicadores", style = "Table Caption")%>% 
  theme_zebra(odd_body = "#EFEFEF", even_header = "transparent",even_body = "transparent") %>% 
  color(color = "#666666", part = "all") %>% 
  flextable::bg(i = 1, bg = "#FFFFFF", part = "header") %>% 
  flextable::bold(bold = TRUE, part = "header") %>% 
...
Sin embargo, sólo a modo ilustrativo, la tabla a continuación presenta de manera sintética y sobre-simplificada cómo sería si todas estas estimaciones puntuales de la coyuntura fueran reunidas en una sola tabla[^procesamiento_fuentes-6]. Reunir todas estas estimaciones se traduce en cientos de miles de filas para el periodo de referencia más reciente solamente.

[^procesamiento_fuentes-6]: La sobre/simplificación obedece a que sólo se presentan algunas columnas, y unos pocos ejemplos (ficticios) de algunas posibles combinaciones de categorías dentro de algunos niveles de agregación.
...
Observando que sigue siendo una simplificación ilustrativa[^procesamiento_fuentes-8], a continuación se presenta lo que podría ser el cálculo coyuntural (módulo I) al 10 de mayo de 2023, cuando teóricamente la DGIET ya accede a los microdatos del IMSS y aun no se publica el primer trimestre de 2023 de la ENOE. 

[^procesamiento_fuentes-8]: Toda vez que en realidad esta tabla con todos los indicadores reunidos en un solo objeto no se genera por el SIDIL -- no hay necesidad de ello. Además, no se presentan el nombre de los campos tal como se utilizan en el sistema, ni todos los campos fueron incluidos en esta tabla.


```{r  echo = FALSE, message=FALSE,warnings=FALSE, results = 'asis'}

tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla10") 

flextable::flextable(tabla1) %>%
   flextable::set_caption( caption = "Presentacion esquematizada de los indicadores coyunturales en sus diferentes niveles de agregacion", style = "Table Caption")%>%
    flextable::set_table_properties(opts_html = list(
    scroll = list(
      height = "900px",
      freeze_first_column = FALSE
...
**A partir del análisis de las tablas resultado de los indicadores coyunturales de cada indicador, se pone en evidencia la facilidad con la que podría construirse un compendio histórico de indicadores**. Esto, en realidad, forma parte del [Módulo IV](#procesamientoModIV); pero conviene presentarlo aquí por completitud. Si a la tabla anterior uno la extendiera en el tiempo para incorporar todas las estimaciones realizadas a lo largo de todos los periodos de referencia previos al último, obtendría como resultado el compendio de indicadores calculados de manera consistente a lo largo del tiempo para las diferentes posibles combinaciones de categorías en los diversos niveles de agregación.

**Esta agregación de estimaciones puntuales *coyunturales* es lo que constituye, ahora sí, la oferta "histórica" de indicadores laborales calculados.** Esto, insistimos, es el histórico de indicadores calculados de manera consistente a partir del SIDIL y permite a un costo muy bajo, acceder a su serie histórica para caracterizar la evolución de ciertos fenómenos laborales. El costo es muy bajo porque tan solo con una rutina de procesamiento adicional es posible identificar todos los outputs de los cálculos coyunturales de cada indicador, "apilarlos" para **reunir  todos los elementos para una rápida representación gráfica y/o geográfica de los fenómenos laborales medidos por SIDIL**. Observando que la construcción de este tipo de tablas no sólo es relativamente sencilla (aunque excede el alcance y necesidades del SIDIL) pero que permite la explotación de información con diferentes propósitos:

    -   Si sólo se tratase de un periodo de tiempo  -- indicadores coyunturales- entonces permite un corte transversal en diferentes niveles de profundización (los niveles de agregación), representando la heterogeneidad en las estimaciones y su respectiva significancia estadística.

    -   Si se tratase de todos o al menos varios periodos temporales, se permite un corte longitudinal: un análisis de la evolución temporal de un indicador (o un conjunto de ellos) en los diferentes niveles de agregación, a lo largo de todos los periodos disponibles.

```{r imagen1, echo=FALSE, out.width="100%", fig.cap="tabla1", eval=FALSE }
knitr::include_graphics("images-1/04/tabla1.png")
```
...
FALTA REEMPLAZAR ESTA TABLA POR IMAGEN

```{r  echo = FALSE, message=FALSE,warnings=FALSE, results = 'asis'}

## 20230822 ya no modificar estilo 
 tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla11")
# kable(tabla1, caption = "Procesos para la actualización del universo de CT con base en el DNE") %>%
#   collapse_rows(c(1,2),latex_hline = "major", valign = "middle") %>% 
#   kable_styling(bootstrap_options = c("striped", "hover"), full_width = T, font_size = 14) %>%
#     # scroll_box(width = "100%", height = "600px") %>% 
#    footnote(number = c("Estos mensajes se muestran en la consola del RStudio al momento de ejecutar el script secuenciador y quedan registrados en la bitácora al momento de finalizar su ejecución."), threeparttable = TRUE)
...
**Los dos procesos descritos anteriormente se realizan en un solo script desarrollado en el lenguaje Python**. El script *"2_matching_coyuntural\_.py"* se encuentra en la carpeta *2_codigos/coyuntura/ cod_matching* correspondiente de códigos. Su aplicación o despliegue se puede llevar a cabo mediante Rstudio.
...
**Por otro lado, el producto del matching coyuntural contendrá tres tipos de columnas que dependen de ciertos factores para determinar la dimensión que tomen**. Primero, el número de atributos que se consultan del DNE, y/o que se imputan a partir de cálculos del SIAPI/SIPAS. Esto está definido por el catálogo de campos que se consultan del DNE[^procesamiento_fuentes-9]. El segundo factor será el número de indicadores que se calculan, mismo que se puede observar en el catálogo de indicadores. El primer caso determinará el número de columnas *atributos*¸mientras que el segundo caso determinará el número de columnas *indicadores.* El tercer factor son los campos de identificación (actualmente *centro_trabajo_id* y *ct_imss_registro* - el número de registro patronal en el IMSS) que, si bien técnicamente están incluidos en el catálogo de campos del DNE, es más intuitivo mantenerlos por separado toda vez que el valor que éstos campos toman no tiene injerencia en el modelo predictivo, sino que son únicamente funcionales a la identificación del CT y a su posible *match* nominativo con el IMSS.

[^procesamiento_fuentes-9]: Recordar que en este catálogo se determina la consulta en más de una entidad (tabla) del SQL Server, y la unión (merge o join) de las mismas requiere campos en común; es decir, hay campos intencionalmente duplicados (que están en diferentes tablas). Notar que a la cuenta de campos en el catálogo hay que quitarle aquellos duplicado para llegar al número total de columnas atributos que conforman el universo de CT con indicadores.

**Tanto las columnas de *atributos,* como las columnas de indicadores, aparecen en su respectivo catálogo, independientemente de cómo se caractericen dichas columnas en su catálogo.** Al respecto, recordar que en el catálogo de campos del DNE hay dos campos que merecen la atención. Primero, el campo *entra_al_modelo.* Este campo es únicamente para definir si los respectivos campos del DNE y los indicadores son variables de entrada para el modelo (ver [Módulo IV](#procesamientoModIV)). Es decir, el valor que esta columna del catálogo de campos del DNE tome no afecta si los campos del DNE entren o no al universo matcheado. Segundo, el campo *tipo*, mismo que determina el tipo de filtro que le aplica a este campo una vez que se genera la MPR en el siguiente paso.
...
El acceso a la interfaz requiere de tener un usuario y contraseña habilitado. En caso de no contar con estas credenciales, contactar a [[XXXX\@stps.gob.mx]{.mark}](mailto:XXXX@stps.gob.mx){.email}[. El acceso a esta interfaz es a través de la siguiente liga XXXX.]{.mark}
...
```{r interfaz1-1,echo=FALSE, out.width="80%", fig.cap = 'Página principal de la Interfaz de Automatización.'}
knitr::include_graphics("images-1/04/interfaz1-1.png")
```
...
```{r interfaz1-2,echo=FALSE, out.width="80%", fig.cap = 'Sección Carga de insumos originales. Pestaña: Carga de insumos originales.'}
knitr::include_graphics("images-1/04/interfaz1-2.png")
```
...
```{r interfaz1-3,echo=FALSE, out.width="80%", fig.cap = 'Sección Selección del periodo. Pestaña: Carga de insumos originales.'}
knitr::include_graphics("images-1/04/interfaz1-3.png")
```
...
```{r interfaz1-4,echo=FALSE, out.width="80%", fig.cap = 'Ventana de diálogo desplegada para seleccionar el archivo del insumo. Sección: Selección del archivo. Pestaña: Carga de insumos originales.'}
knitr::include_graphics("images-1/04/interfaz1-4.png")
```
...
```{r interfaz1-5,echo=FALSE, out.width="80%", fig.cap = 'Avance de carga del archivo. Sección: Selección del archivo. Pestaña: Carga de insumos originales.'}
knitr::include_graphics("images-1/04/interfaz1-5.png")
```
...
```{r interfaz1-6,echo=FALSE, out.width="80%", fig.cap = 'Sección Resumen de la Carga y guardado de archivos seleccionados. Pestaña: Carga de insumos originales.'}
knitr::include_graphics("images-1/04/interfaz1-6.png")
```
...
```{r interfaz1-7,echo=FALSE, out.width="80%", fig.cap = 'Pestaña de carga de catálogos.'}
knitr::include_graphics("images-1/04/interfaz1-7.png")
```
...
```{r interfaz1-8,echo=FALSE, out.width="80%", fig.cap = 'Sección de carga de archivos. Pestaña Carga de catálogos.'}
knitr::include_graphics("images-1/04/interfaz1-8.png")
```
...
```{r interfaz1-9,echo=FALSE, out.width="80%", fig.cap = 'Carga de un nuevo archivo. Sección de carga. Pestaña de Carga de catálogos.'}
knitr::include_graphics("images-1/04/interfaz1-9.png")
```
...
```{r interfaz1-10,echo=FALSE, out.width="80%", fig.cap = 'Mensaje de confirmación de carga de catálogos. Pestaña de Carga de catálogos.'}
knitr::include_graphics("images-1/04/interfaz1-10.png")
```
...
```{r interfaz1-11,echo=FALSE, out.width="80%", fig.cap = 'Sección de Estatus de fuestes y procesos. Pestaña de Estatus y ejecución de procesos.'}
knitr::include_graphics("images-1/04/interfaz1-11.png")
```
...
```{r interfaz1-12,echo=FALSE, out.width="80%", fig.cap = 'Sección Procesos. Pestaña de Estatus y ejecución de procesos.'}
knitr::include_graphics("images-1/04/interfaz1-12.png")
```
...
```{r interfaz1-13,echo=FALSE, out.width="80%", fig.cap = 'Sección de Procesamiento de información. Pestaña de Estatus y ejecución de procesos.'}
knitr::include_graphics("images-1/04/interfaz1-13.png")
```
...
```{r interfaz1-14,echo=FALSE, out.width="80%", fig.cap = 'Consola de seguimiento. Sección de Procesamiento de información. Pestaña de Estatus y ejecución de procesos.'}
knitr::include_graphics("images-1/04/interfaz1-14.png")
```
...
```{r interfaz1-15,echo=FALSE, out.width="80%", fig.cap = 'Mensaje de proceso en ejecución o corriendo. Sección de Procesamiento de información. Pestaña de Estatus y ejecución de procesos.'}
knitr::include_graphics("images-1/04/interfaz1-15.png")
```
...
```{r interfaz1-16,echo=FALSE, out.width="80%", fig.cap = 'Sección Descarga de información. Pestaña de Estatus y ejecución de procesos.'}
knitr::include_graphics("images-1/04/interfaz1-16.png")
```
...
```{r interfaz2-2,echo=FALSE, out.width="80%", fig.cap = 'Página principal de la interfaz.'}
knitr::include_graphics("images-1/04/interfaz2-2.png")
```
...
```{r interfaz2-3,echo=FALSE, out.width="80%", fig.cap = 'Primera pestaña: Filtrado.'}
knitr::include_graphics("images-1/04/interfaz2-3.png")
```
...
```{r interfaz2-4,echo=FALSE, out.width="80%", fig.cap = 'Sección Predictores. Pestaña Filtrado.'}
knitr::include_graphics("images-1/04/interfaz2-4.png")
```
...
```{r interfaz2-5,echo=FALSE, out.width="80%", fig.cap = 'Opción de quitar selección. Sección Predictores. Pestaña Filtrado.'}
knitr::include_graphics("images-1/04/interfaz2-5.png")
```
...
```{r interfaz2-6,echo=FALSE, out.width="80%", fig.cap = 'Sección Indicadores. Pestaña Filtrado.'}
knitr::include_graphics("images-1/04/interfaz2-6.png")
```
...
```{r interfaz2-7,echo=FALSE, out.width="80%", fig.cap = 'Sección Atributos. Pestaña Filtrado.'}
knitr::include_graphics("images-1/04/interfaz2-7.png")
```
...
```{r interfaz2-8,echo=FALSE, out.width="80%", fig.cap = 'Aleatorización. Pestaña Filtrado.'}
knitr::include_graphics("images-1/04/interfaz2-8.png")
```
...
```{r interfaz2-9,echo=FALSE, out.width="80%", fig.cap = 'Pestaña Análisis.'}
knitr::include_graphics("images-1/04/interfaz2-9.png")
```
...
```{r interfaz2-10,echo=FALSE, out.width="80%", fig.cap = 'Sección Distribución de Centros de Trabajo por Estado. Pestaña Análisis.'}
knitr::include_graphics("images-1/04/interfaz2-10.png")
```
...
```{r interfaz2-11,echo=FALSE, out.width="80%", fig.cap = 'Sección Distribución de Centros de Trabajo por Tamaño. Pestaña Análisis.'}
knitr::include_graphics("images-1/04/interfaz2-11.png")
```
...
```{r interfaz2-12,echo=FALSE, out.width="80%", fig.cap = 'Sección Estadísticas de la matriz. Pestaña Análisis.'}
knitr::include_graphics("images-1/04/interfaz2-12.png")
```
...
```{r interfaz2-13,echo=FALSE, out.width="80%", fig.cap = 'Exportar centros de trabajo seleccionados. Pestaña Filtrado.'}
knitr::include_graphics("images-1/04/interfaz2-13.png")
```
...
```{r interfaz2-14,echo=FALSE, out.width="80%", fig.cap = 'Tabla final de resultados. Pestaña matriz.'}
knitr::include_graphics("images-1/04/interfaz2-14.png")
```
...
```{r interfaz2-15,echo=FALSE, out.width="80%", fig.cap = 'Tabla final de resultados. Pestaña predictor.'}
knitr::include_graphics("images-1/04/interfaz2-15.png")
```
...
```{r interfaz2-16,echo=FALSE, out.width="80%", fig.cap = 'Tabla final de resultados. Pestaña indicadores.'}
knitr::include_graphics("images-1/04/interfaz2-16.png")
```

```{r interfaz2-17,echo=FALSE, out.width="80%", fig.cap = 'Tabla final de resultados. Pestaña atributos.'}
knitr::include_graphics("images-1/04/interfaz2-17.png")
```
...
```{r parametrosimsshist, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="Configuración de parámetros del script secuenciador del IMSS para el cálculo histórico", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
```

```{block, type='rmdcomment', fig.cap="Configuración de parámetros del script secuenciador del IMSS para el cálculo histórico"}
...
```{r modelos, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="Modelos Random Forest y Tabnet", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
```

```{block, type='rmdcomment', fig.cap="Modelos Random Forest y Tabnet"}
...
-   El script de entrenamiento del modelo tiene las siguientes partes:
    ...
    2.  Funciones de configuración, entre las que se encuentran procesos como la definición de rutas, definición de fecha y hora, de apertura de log, tamaño de *chunk*[^procesamiento_fuentes-10], entre otros.
    ...
    b.  TabNet: Primero se generan los archivos .txt y .pkl que albergarán los resultados del modelo, los cuales será utilizados en la predicción. Estos son:
    ...
    9.  Se guardan los resultados de la predicción y se ajusta el base resultado del riesgo total.
    10. Se registra el proceso en las bitácoras correspondientes.
    11. Cierre.

-   El script de entrenamiento del modelo tiene las siguientes partes:

    1.  Generación de métricas de *performance* del modelo predictivo entrenado para los dos modelos, ya sea Random Forest o TabNet. En esta parte se generan dos tablas que se imprimirán en el log reportando las métricas de rendimiento del modelo, estas son^[En el caso del modelo Random Forest MultiOutput, se muestran estas mismas medidas para cada una de las predicciones, además de un promedio general. Para el caso de TabNet se muestran otras medidas propias de los modelos de regresión, como R2, mínimos cuadraros (MCO) y error cuadrático medio (RMSE).]:
    
    - Exactitud (accuracy): La exactitud es una medida de qué tan bien el modelo predictor coincide con los datos reales, en otras palabras mide cuánto se aproximan los resultados obtenidos a los valores verdaderos, tanto positivos como negativos.  
      $Accuracy = \frac{VP + VN}{VP + VN + FP + FN}$ ^[VP:verdadero positivo; VN: verdadero negativo; FP: falso positivo; FN: falso negativo]

    -   Puntaje de precisión (precision score): Es una métrica que mide la proporción de predicciones positivas correctas (verdaderos positivos) con respecto a todas las predicciones positivas realizadas por el modelo, es decir,  proporciona información sobre su rendimiento con respecto a los falsos positivos.
      $Precision = \frac{VP}{VP + FP}$ 

    -   Sensibilidad o tasa de verdaderos positivos (recall): Es una medida que evalúa la capacidad de un modelo de clasificación para identificar correctamente las muestras positivas. El recall se centra en el rendimiento de un clasificador con respecto a falsos negativos (cuántos fallaron).
...
[^procesamiento_fuentes-10]: Se define como chunk las partes en las que se divide la tabla de datos para realizar los procesos necesarios de manera iterativa, esto con el fin de lograr mayor eficiencia al ejecutar los procesos con grandes cantidades de datos. Así, chunksize es la definición del tamaño de cada parte.
...
```{r nuevassubmaterias, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="Inclusión de submaterias para la predicción", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
```

```{block, type='rmdcomment', fig.cap="Inclusión de submaterias para la predicción"}
...
Todos estos archivos son necesarios para la aplicación del modelo en el [Módulo III](##procesamientoModIIIinterfazConsult).
...
```{r shap, class.source='myClass', echo=FALSE, out.width="50px", fig.cap="Reporte Shapley", out.extra='style="float:left; background-color: #fff; padding-right:1em"'}
knitr::include_graphics("images-1/important-icon.svg")
```

```{block, type='rmdcomment', fig.cap="Reporte Shapley"}
...
El análisis Shapley se utiliza para comprender la contribución de características individuales en un modelo de aprendizaje automático y cómo afectan las predicciones del modelo. Dado que modelos como Random Forest o TabNet no arrojan resultados de contribución a la predicción de las variables de la misma manera que lo hace una regresión simple, por ejemplo, el análisis Shapley se aplica con el fin de comprender cómo las características individuales afectan las predicciones del modelo.
 ...
 -   SHAP Feature Importance: Promedio de los valores absolutos para obtener una importancia global de cada uno de los atrbutos. Ejemplo:
     ...
     ![Ejemplo SHAP Feature Importance](images-1/04/shapley1.png)
 ...
 -   Diagrama de resumen SHAP: Este diagrama combina la importancia de las características con los efectos de las características. Cada punto en la gráfica de resumen es un valor de Shapley para una característica y una instancia. La posición en el eje y está determinada por la característica y en el eje x por el valor de Shapley. El color representa el valor de la característica de menor a mayor. Ejemplo:
     ...
     ![Ejemplo Diagrama de resumen SHAP](images-1/04/shapley2.png)
 ...
 -   Diagramas de cascada: Estos diagramas están diseñados para mostrar explicaciones de predicciones individuales. La parte inferior del diagrama comienza como el valor esperado o Treshhold de la salida del modelo ($(E[f(X)]$) y luego cada fila muestra cómo la contribución positiva (roja) o negativa (azul) de cada característica mueve el valor de la salida esperada del modelo sobre el conjunto de datos del modelo hasta alcanzar la predicción ($(f(x))$). Ejemeplo:
     ...
     ![Ejemplo Diagrama de cascada](images-1/04/shapley3.png)
