...
# kable(tabla1, caption = "Resumen de las fuentes de información utilizadas en el SIDIL") %>%
#   kable_styling(bootstrap_options = c("striped", "hover"), full_width = T, font_size = 14) %>%
#     scroll_box(width = 
#                  "100%", height = "600px") #%>%
# footnote(general = "Los detalles de las fuentes de informacion se presentan en el Modulo II")
```
...
```{r echo = FALSE, results = 'asis'}
# tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla2")
# kable(tabla1, caption = "Indicadores calculados con base en la ENOE") %>%
#   kable_styling(bootstrap_options = c("striped", "hover"), full_width = T, font_size = 14) %>%
...
Aunque la estructura del nombre puede variar, es importante que el nombre contega los siguientes elementos para que sean detectados por el script extractor: nombre del cuestionario (COE1, COE2 o SDEM), un dígito para el trimestre, seguido por una T, seguido por dos dígitos para el año, y finalizar con el forma
...
# 
# tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla3")
# 
# 
# kable(tabla1, caption = "Procesos para la actualización de indicadores con base en la ENOE") %>% collapse_rows(c(1,2,3),latex_hline = "major", valign = "middle") %>% 
#   kable_styling(bootstrap_options = c("striped", "hover"), full_width = T, font_size = 14) %>% 
#      scroll_box(width = "100%", height = "600px") 

#%>% 
  # footnote(number = c("Los mensajes confirmatorios y de error o advertencia se muestran en la consola del RStudio al momento de ejecutar el script secuenciador y quedan registrados en la bitácora al momento de finalizar su ejecució.
  #                     " ,"El mensaje de confirmación referente a la población subordinada y remunerada de la ENOE permite cotejar la información producida por el script con la información oficial del INEGI, para corroborar que se haya generado de manera correcta la base de datos y que se esté utilizando el último insumo de la encuesta.
  #                     ", "El error referente a la no disponibilidad de indicadores surgirá del segundo al cuarto trimestre del año en el indicador “Sindicato” de la ENOE, ya que se calcula de manera anual con el cuestionario ampliado del primer trimestre del año."), threeparttable = TRUE)
```
...
```{r detallescriptsENOE, echo=FALSE, out.width="100%", fig.cap="detallescriptsENOE"}
...
```{r echo = FALSE, results = 'asis'}
# tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla4")
# kable(tabla1, caption = "Indicadores que se calculan con base en los datos del IMSS") %>%
#   kable_styling(bootstrap_options = c("striped", "hover"), full_width = T, font_size = 14) %>%
#     scroll_box(width = "100%", height = "600px")
```
...
tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla5") %>% janitor::clean_names()
flextable::flextable(tabla1) %>%
  merge_custom(x = tabla1$ejecucion_del_usuario, columns=1:3) %>%
  flextable::theme_box() %>% 
   flextable::set_caption( caption = "yyyyyy Procesos para la actualización de indicadores con base en los datos del IMSS", 
                  style = "Table Caption")%>% 
  flextable::footnote( i = 1, j = 4,  ref_symbols = "a",
  value = flextable::as_paragraph("Estos mensajes se muestran en la consola del RStudio al momento de ejecutar el script secuenciador y quedan registrados en la bitácora al momento de finalizar su ejecución.")
) %>% 
...
## ESTA TABLA FUE REEMPLAZADA

# tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla5")
# kable(tabla1, caption = "yyyyyy Procesos para la actualización de indicadores con base en los datos del IMSS") %>% 
#   kable_styling(bootstrap_options = c("striped", "hover"), full_width = T, font_size = 14) %>%
#     scroll_box(width = "100%", height = "600px") %>%
...
#   kableExtra::collapse_rows(c(1,2,3))#,latex_hline = "major", valign = "middle") 
```
...
# kable(tabla1, caption = "Indicadores que se calculan con base en el Censo Económico") %>%
#   kable_styling(bootstrap_options = c("striped", "hover"), full_width = T, font_size = 14)
```
...
# kable(tabla1, caption = "Campos que se utilizan para el cálculo de indicadores con base en el Censo Económico") %>%
#   kable_styling(bootstrap_options = c("striped", "hover"), full_width = T, font_size = 14) %>%
#     scroll_box(width = "100%", height = "600px")
```

```{r detallescriptsIMSS, echo=FALSE, out.width="100%", fig.cap="detallescriptsIMSS"}
...
```{r echo = FALSE, results = 'asis'}
# tabla1 <- read_excel("./data/tablas03.xlsx", sheet = "tabla8")
# kable(tabla1, caption = "Procesos para la actualización de indicadores con base en el censo económico") %>%
#   kable_styling(bootstrap_options = c("striped", "hover"), full_width = T, font_size = 14) %>% 
#       scroll_box(width = "100%", height = "500px")
```

```{r detallescriptsCENSO, echo=FALSE, out.width="100%", fig.cap="detallescriptsCENSO"}
knitr::include_graphics("images-1/04/detallescriptsCENSO.jpg")
```

##### Posibles problemas específicos en la aplicación de scripts {#problemascenso .unnumbered}
...
##### Salidas - Outputs productos resultado de la ejecución de actividades {#outputcenso .unnumbered}
...
# kable(tabla1, caption = "Campos comunes en los resultados coyunturales del cálculo de indicadores") %>%
#   kable_styling(bootstrap_options = c("striped", "hover"), full_width = T, font_size = 14) %>%
#     scroll_box(width = "100%", height = "600px")
```
...
# kable(tabla1, caption = "Presentación esquematizada de los indicadores coyunturales en sus diferentes niveles de agregación") %>%
# collapse_rows(c(1,2,3),latex_hline = "major", valign = "middle") %>% 
#   kable_styling(bootstrap_options = c("striped", "hover"), full_width = T, font_size = 14) %>%
#     scroll_box(width = "100%", height = "600px")
```
...
```{r tabla1,echo=FALSE, out.width="80%", fig.cap = 'Presentación esquematizada del resultado del cálculo de indicadores de manera histórica.'}
...
#### Salidas - Outputs - Productos resultado de la ejecución de actividades {#outputmatch .unnumbered}
...
#### Mensajes generados {#mensajesmath .unnumbered}
...
-   El script de entrenamiento del modelo tiene las siguientes partes:
    ...
    1.  Generación de métricas de *performance* del modelo predictivo entrenado para los dos modelos, ya sea Random Forest o TabNet. En esta parte se generan dos tablas que se imprimirán en el log reportando las métricas de rendimiento del modelo, estas son^[En el caso del modelo Random Forest MultiOutput, se muestran estas mismas medidas para cada una de las predicciones, además de un promedio general. Para el caso de TabNet se muestran otras medidas propias de los modelos de regresión, como R2, mínimos cuadraros (MCO) y error cuadrático medio (RMSE).]:
        -   Exactitud (accuracy): La exactitud es una medida de qué tan bien el modelo predictor coincide con los datos reales, en otras palabras mide cuánto se aproximan los resultados obtenidos a los valores verdaderos, tanto positivos como negativos.  
         $\Accuracy = VP + VN / VP + VN + FP + FN$ ^[VP:verdadero positivo; VN: verdadero negativo; FP: falso positivo; FN: falso negativo]
